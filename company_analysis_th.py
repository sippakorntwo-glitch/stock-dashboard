...
METRIC_TEXT = {
    'marketCap': ('มูลค่าหลักทรัพย์ตามราคาตลาด (Market Cap)', 'ราคาหุ้นคูณจำนวนหุ้นที่ออกและถืออยู่ ขนาดของบริษัทไม่ได้บอกคุณภาพของกิจการโดยลำพัง'),
    'enterpriseValue': ('มูลค่ากิจการ (EV)', 'มูลค่ากิจการตามผู้ให้ข้อมูล รวมมูลค่าหุ้นตามราคาตลาดและภาระที่มีลักษณะคล้ายหนี้ แล้วหักเงินสด ควรตรวจสอบวิธีคำนวณของผู้ให้ข้อมูล'),
    'trailingPE': ('ราคาต่อกำไรย้อนหลัง (P/E)', 'ราคาหุ้นหารกำไรต่อหุ้นย้อนหลัง หากกำไรเป็นศูนย์หรือติดลบ อัตราส่วนนี้จะตีความไม่ได้ ค่า P/E ต่ำอาจสะท้อนความเสี่ยงได้เช่นกัน'),
    'forwardPE': ('ราคาต่อกำไรคาดการณ์ (Forward P/E)', 'ราคาหุ้นหารกำไรต่อหุ้นที่คาดการณ์ ประมาณการของนักวิเคราะห์เปลี่ยนแปลงได้และยังไม่ใช่กำไรที่เกิดขึ้นจริง'),
    'priceToBook': ('ราคาต่อมูลค่าทางบัญชี (P/B)', 'ราคาหุ้นหารมูลค่าทางบัญชีต่อหุ้น หรือ P/BV หากส่วนของผู้ถือหุ้นติดลบ อัตราส่วนนี้จะตีความไม่ได้ ธุรกิจที่ใช้สินทรัพย์น้อยและธนาคารควรใช้บริบทเปรียบเทียบต่างกัน'),
    'priceToSales': ('ราคาต่อยอดขาย (P/S)', 'มูลค่าหุ้นตามราคาตลาดเทียบกับยอดขายย้อนหลัง ยอดขายเพียงอย่างเดียวไม่ได้บอกความสามารถทำกำไร'),
    'evRevenue': ('มูลค่ากิจการต่อรายได้ (EV/Revenue)', 'มูลค่ากิจการเทียบกับรายได้ ควรเปรียบเทียบธุรกิจที่มีอัตรากำไร การเติบโต และนโยบายบัญชีใกล้เคียงกัน'),
    'evEbitda': ('มูลค่ากิจการต่อ EBITDA', 'มูลค่ากิจการหารกำไรก่อนดอกเบี้ย ภาษี ค่าเสื่อมราคาและค่าตัดจำหน่าย EBITDA ยังไม่หักรายจ่ายลงทุนและความต้องการเงินทุนหมุนเวียน จึงไม่ใช่กระแสเงินสดอิสระ'),
    ...
    'returnOnEquity': ('ผลตอบแทนต่อส่วนของผู้ถือหุ้น (ROE)', 'กำไรสุทธิหารส่วนของผู้ถือหุ้นเฉลี่ยเมื่อมีงบที่ตรงกัน หากไม่มีจะใช้ ROE จากผู้ให้ข้อมูล การซื้อหุ้นคืน หนี้สูง หรือฐานทุนเล็กอาจทำให้ ROE สูงขึ้น'),
    'returnOnAssets': ('ผลตอบแทนต่อสินทรัพย์ (ROA)', 'กำไรสุทธิหารสินทรัพย์เฉลี่ยเมื่อมีงบที่ตรงกัน หากไม่มีจะใช้ ROA จากผู้ให้ข้อมูล ควรเทียบธุรกิจที่ใช้สินทรัพย์มากน้อยใกล้เคียงกัน'),
    'roic': ('ผลตอบแทนต่อเงินลงทุน (ROIC)', 'กำไรจากการดำเนินงานหลังภาษีหารเงินลงทุนเฉลี่ย ในที่นี้เงินลงทุนคือส่วนของผู้ถือหุ้นบวกหนี้ที่มีดอกเบี้ย แล้วหักเงินสดและเงินลงทุนระยะสั้น ใช้อัตราภาษีที่รายงานจริงเฉพาะเมื่ออยู่ในช่วง 0–100% ค่าประมาณนี้อาจต่างจาก ROIC ที่นักวิเคราะห์ปรับปรุง ควรเทียบกับต้นทุนเงินทุนถัวเฉลี่ยถ่วงน้ำหนักของบริษัท (WACC) โดยระบบไม่ได้สมมติ WACC'),
    ...
    'revenueGrowthFY': ('การเติบโตของรายได้ทั้งปีบัญชี (FY)', 'รายได้ทั้งปีบัญชีล่าสุดหารรายได้ทั้งปีบัญชีก่อน แล้วลบหนึ่ง ใช้ตัวเลขทั้งสองปีจากงบกำไรขาดทุน'),
    'netIncomeGrowthFY': ('การเติบโตของกำไรสุทธิทั้งปีบัญชี (FY)', 'กำไรสุทธิทั้งปีบัญชีล่าสุดหารกำไรสุทธิทั้งปีบัญชีก่อน แล้วลบหนึ่ง หากปีก่อนมีกำไรเป็นศูนย์หรือขาดทุนจะแสดงว่าตีความไม่ได้ ไม่แสดงเป็นอัตราเติบโตตามปกติ'),
    'epsGrowthFY': ('การเติบโตของกำไรต่อหุ้นปรับลดทั้งปี (EPS)', 'กำไรต่อหุ้นปรับลดทั้งปีบัญชีล่าสุดหารค่าของปีก่อน แล้วลบหนึ่ง ไม่ประเมินอัตราเติบโตเมื่อฐานปีก่อนเป็นศูนย์หรือติดลบ'),
    ...
    'ebit': ('กำไรก่อนดอกเบี้ยและภาษี (EBIT)', 'กำไรก่อนดอกเบี้ยและภาษีที่รายงาน ไม่ใช้ EBITDA แทนและไม่ประมาณจากอัตรากำไร'),
    'ebitda': ('กำไรก่อนดอกเบี้ย ภาษี ค่าเสื่อมและค่าตัดจำหน่าย (EBITDA)', 'กำไรก่อนดอกเบี้ย ภาษี ค่าเสื่อมราคาและค่าตัดจำหน่าย EBITDA ที่เป็นบวกไม่ได้รับประกันว่าจะสร้างเงินสดเป็นบวก'),
    ...
    'dilutedEPS': ('กำไรต่อหุ้นปรับลด (Diluted EPS)', 'กำไรต่อจำนวนหุ้นถัวเฉลี่ยถ่วงน้ำหนักแบบปรับลด ใช้ EPS รายปีที่รายงานโดยตรง ไม่บวก EPS รายไตรมาสเพื่อสร้างตัวเลขรายปี'),
    'debtToEquity': ('หนี้ที่มีดอกเบี้ยต่อส่วนของผู้ถือหุ้น (D/E)', 'หนี้ที่มีดอกเบี้ยรวมหารส่วนของผู้ถือหุ้น แสดงหน่วยเป็นเท่า ค่า debtToEquity ในข้อมูลสรุปของ Yahoo เป็นร้อยละ จึงหาร 100 ก่อนแสดง ต่างจากหนี้สินรวมต่อส่วนของผู้ถือหุ้น หากส่วนของผู้ถือหุ้นเป็นศูนย์หรือติดลบจะตีความไม่ได้'),
    ...
    'currentRatio': ('อัตราส่วนเงินทุนหมุนเวียน (Current Ratio)', 'สินทรัพย์หมุนเวียนหารหนี้สินหมุนเวียน ค่าต่ำกว่า 1 อาจสะท้อนแรงกดดันด้านสภาพคล่อง แต่ค่าสูงไม่ได้หมายถึงประสิทธิภาพที่สูงเสมอ เกณฑ์นี้ไม่เหมาะกับธนาคาร'),
    'quickRatio': ('อัตราส่วนสภาพคล่องหมุนเร็ว (Quick Ratio)', 'ในที่นี้คือเงินสด รายการเทียบเท่าเงินสด เงินลงทุนระยะสั้น และลูกหนี้ หารหนี้สินหมุนเวียน หากใช้ค่าจากผู้ให้ข้อมูล นิยามสินทรัพย์หมุนเร็วอาจต่างกัน ควรตรวจสอบแหล่งข้อมูล'),
    'cashRatio': ('อัตราส่วนเงินสด (Cash Ratio)', 'เงินสด รายการเทียบเท่าเงินสด และเงินลงทุนระยะสั้น หารหนี้สินหมุนเวียน เป็นมุมมองสภาพคล่องที่จำกัดรายการมากกว่าอัตราส่วนเงินทุนหมุนเวียน'),
    ...
    'operatingCashflow': ('กระแสเงินสดจากการดำเนินงาน (OCF)', 'เงินสดที่ได้มาหรือใช้ไปจากการดำเนินงานในงวดที่ระบุ ควรตรวจสอบการเปลี่ยนแปลงเงินทุนหมุนเวียนและแยกผลที่เกิดประจำออกจากผลชั่วคราว'),
    'capex': ('รายจ่ายลงทุน (CapEx)', 'เงินสดที่จ่ายเพื่อลงทุนในสินทรัพย์ระยะยาว แสดงรายจ่ายเป็นค่าบวก หากไม่มีข้อมูลเงินสดจ่ายต้นทางจะไม่ถือว่าเป็นศูนย์'),
    'freeCashflow': ('กระแสเงินสดอิสระ (FCF)', 'กระแสเงินสดอิสระที่รายงาน หรือกระแสเงินสดจากการดำเนินงานหักรายจ่ายลงทุนเมื่อข้อมูลทั้งสองตรงงวดกัน ไม่ได้เป็นเงินสดที่สามารถจ่ายให้ผู้ถือหุ้นได้ทั้งหมดโดยอัตโนมัติ'),
    ...
    'cashConversion': ('การเปลี่ยนกำไรเป็นเงินสด (OCF/กำไรสุทธิ)', 'กระแสเงินสดจากการดำเนินงานหารกำไรสุทธิที่เป็นบวกในงวดเดียวกัน หากต่ำกว่า 1 อย่างต่อเนื่องควรตรวจสอบ แต่จังหวะเงินทุนหมุนเวียนอาจทำให้ผันผวนชั่วคราวได้'),
    ...
    'forwardEps': ('กำไรต่อหุ้นคาดการณ์ (Forward EPS)', 'กำไรต่อหุ้นคาดการณ์จากผู้ให้ข้อมูล เป็นประมาณการซึ่งต่างจากกำไรต่อหุ้นปรับลดทั้งปีบัญชีที่รายงานจริง'),
}
