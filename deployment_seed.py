"""Prime commonly viewed symbols after deployment without replacing other saved data."""
...
def main():
    if os.environ.get('GITHUB_ACTIONS') != 'true':
        raise RuntimeError('Deployment seed is intended for GitHub Actions only')
    ...
    if cache.error:
        raise RuntimeError('Cannot persist local cache')
    ...
    if not previous and not original.empty:
        cache.save_quotes(original)
    ...
    manifest = publish_snapshot(store, cache, universe, report, previous,
                                watchlist_csv=data.decode('utf-8-sig') if data else None)
    print('DEPLOYMENT_SEED', json.dumps({'generation': manifest['generation'], 'coverage': manifest['coverage'],
                                      'remaining': manifest['bootstrap_pending'], 'report': report}, ensure_ascii=False), flush=True)
    required = ('AAPL', 'SPY')
    missing = []
    for ticker in required:
        history, _ = cache.history(ticker)
        if history is None:
            missing.append(ticker + ':history')
        for kind in ('info', 'dividends'):
            value, _ = cache.get(kind + ':' + ticker, request_remote=False)
            if value is None:
                missing.append(ticker + ':' + kind)
    ...
    if missing:
        raise RuntimeError('Available progress published, but required initial data is missing: ' + ', '.join(missing))
