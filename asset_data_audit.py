...
def enrich(cache,universe,etfs,profiles,limit=40):
    ...
    missing_accounts=[t for t in universe if t not in etfs and any(not present(profiles.get('info:'+t,({},{}))[0].get(f)) for f in ('operatingCashflow','freeCashflow','totalCash'))]
