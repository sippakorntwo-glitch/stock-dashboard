...
.inspector-row[data-field=buy_volume_est] b{color:#26a69a}.inspector-row[data-field=sell_volume_est] b{color:#ef5350}
...
Object.assign(inspectionNames,{buy_volume_est:'ซื้อประมาณ (จากราคา)',sell_volume_est:'ขายประมาณ (จากราคา)',unclassified_volume:'Volume ที่แยกไม่ได้'});
...
function inspectionFormat(k,v){
 ...
 const digits=['volume','buy_volume_est','sell_volume_est','unclassified_volume'].includes(k)?0:k==='rsi'||k==='barChange'?2:['macd','signal','hist'].includes(k)?Math.max(4,p.precision):p.precision;
...
function inspectionFields(r){
 ...
 if(opts.volume&&opts.volumeSplit)list.push('buy_volume_est','sell_volume_est','unclassified_volume');
...
function inspectionSeries(){
 ...
 let pane=1;
 if(opts.volume){result.push(['volume',series.volume,pane]);if(opts.volumeSplit)result.push(['buy_volume_est',series.buy_volume_est,pane]);pane++;}
...
function inspectionDraw(point){
 ...
 if(opts.volume&&opts.volumeSplit)tooltip.append(inspectionNode('div','inspector-hint','ซื้อ/ขายเป็นค่าประมาณจากตำแหน่งราคาปิด ไม่ใช่ข้อมูลผู้เริ่มซื้อขายจริง'));
