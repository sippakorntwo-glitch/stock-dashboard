...
def review_html(review):
    ...
    for group in GROUPS:
        ...
        for r in rows:
            ...
            body.append(f'<tr data-metric="{escape(r["key"],quote=True)}" data-observation="{escape(evidence,quote=True)}"><td><abbr tabindex="0" title="{escape(tip,quote=True)}">{escape(r["metric"])} <small>ⓘ</small></abbr></td><td>{escape(format_value(r))}</td><td>{escape(r["guide"])}</td><td><span class="grade {tone}">{escape(grade)}</span><br>{escape(r["interpretation"])}</td><td class="period">{escape(r["basis"])}</td></tr>')
