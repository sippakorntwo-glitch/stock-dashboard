"""Conservative entry gates, additional to (not a replacement for) the 100-point model.

Unknown checks block an entry; watch candidates remain visible separately. These
are disclosed heuristics, not a comprehensive due-diligence or profit guarantee.
"""
...
def entry_checks(row, now=None):
    ...
    def positive(field, name):
        v=finite(info.get(field)); add(name, v is not None and v>0, 'ต้องเป็นบวก; ไม่มีข้อมูลไม่ถือว่าผ่าน')
    ...
    if row.get('asset_type')=='ETF':
        ...
        add('รายได้ไม่หดตัว',growth is not None and growth>=0,
            'revenueGrowth ≥0 ตามช่วงที่ผู้ให้ข้อมูลรายงาน')
        debt,cash,ocf=(finite(info.get(k)) for k in ('totalDebt','totalCash','operatingCashflow'))
        ratio=max(0,debt-cash)/ocf if debt is not None and cash is not None and ocf is not None and debt>=0 and cash>=0 and ocf>0 else None
...
def apply_entry_policy(rows, now=None):
    ...
    for original in rows:
        row=dict(original); audit=entry_checks(row,now)
        row['entry_audit']=audit
        row['ready_at_calculation']=bool(row.get('ready_at_calculation') and audit['passed'])
        row['blockers']=list(dict.fromkeys([*row.get('blockers',[]),
            *['ตรวจเพิ่ม: '+x['check'] for x in audit['checks'] if not x['passed']]))
        result.append(row)
...
def is_entry(row, payload, now=None):
    ...
    # Only current-policy publications can confirm entries. Legacy feeds fail closed.
