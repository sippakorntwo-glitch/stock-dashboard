"""GitHub Release assets + an atomic small manifest in a separate data branch.

No Actions artifacts, LFS, Packages, cloud buckets or paid storage APIs.
Public readers download raw files directly, without a token or GitHub REST calls.
"""
...
class PublicGitHubStore:
    """Read-only public files. No Authorization header, REST API, or mutation."""
    def __init__(self, config, session=None):
        ...
        # Never inherit .netrc credentials or a caller's session Authorization header.
...
class GitHubReleaseStore:
    ...
    def _request(self, method, path, *, missing=False, upload=False, **kwargs):
        # Construct URLs ourselves. Never send the token to a URL supplied by a manifest.
        if not path.startswith("/") or "://" in path or ".." in path:
            raise ValueError("Invalid GitHub API path")
        if method != "GET":
            # GitHub has secondary content-creation limits; space out mutations.
            self._pace_mutation()
        ...
        try:
            response = self.session.request(method, base + path, headers=headers,
                                            timeout=(10, 120), **kwargs)
        except requests.RequestException as exc:
            # Request exceptions may include signed redirect URLs; don't surface them.
        ...
        if not 200 <= response.status_code < 300:
            status = response.status_code
            response.close()
            raise GitHubStoreError(f"GitHub HTTP {status}; check token permissions, expiry and rate limits")
        return response
    ...
    def _index(self, tag):
        ...
        if tag not in self.assets:
            index, page = {}, 1
            while True:
                batch = self._json("GET", f"/releases/{int(release['id'])}/assets",
    ...
    def _asset_response(self, key):
        ...
        return self._request("GET", f"/releases/assets/{int(asset['id'])}",
                             headers={"Accept": "application/octet-stream"}, stream=True)

    def read(self, key, optional=False):
        self._verify_repository()
        if key == "latest.json":
            item = self._json("GET", "/contents/" + POINTER_PATH, missing=optional,
            ...
            # Bound caches: each new generation needs at most a few recent releases.
    ...
    def write(self, key, raw):
        ...
        if release.get("draft"):
            self.releases[tag] = self._json("PATCH", f"/releases/{int(release['id'])}",
        ...
        # GitHub enforces the previous SHA. Concurrent writers cannot silently replace it.
    ...
    def prune_generations(self, manifest, keep_days=3):
        self._verify_repository()
        # Check the current pointer immediately before removing old generations.
        actual = json.loads(self.read("latest.json"))
        if actual["generation"] != manifest["generation"]:
            return
        protected = {TAG_PREFIX + g.split("/")[-1] for g in
                     (actual["generation"], actual.get("previous_generation")) if g}
        releases, page = [], 1
        while True:
