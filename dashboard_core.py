"""Preserved calculation/chart helpers from the saved dashboard v2026-09-10.8.

Imported by dashboard_runtime; app.py is the Streamlit entrypoint.
"""
...
@st.cache_resource
def _data_cache_for_version(version):
    return DashboardCache(CACHE_FILE)


def get_data_cache(_version=APP_VERSION):
    return _data_cache_for_version(_version)


get_data_cache.clear = _data_cache_for_version.clear


@st.cache_resource
def _remote_reader_for_version(version):
    """Optional prepared-data mode. Credentials stay on the server."""
    try:
        ...
        return None, f"ยังเปิดโหมดข้อมูลอัตโนมัติไม่ได้ ({type(exc).__name__}) ตรวจไฟล์ data_sync.py, requirements.txt และ GitHub Secrets / การตั้งค่าโหมดบนคอมตามคู่มือ"


def get_remote_reader(_version=APP_VERSION):
    return _remote_reader_for_version(_version)


get_remote_reader.clear = _remote_reader_for_version.clear


def render_prepared_status(reader):
    state = reader.status()
    ...
    if not manifest:
        st.caption("ระหว่างรอชุดแรก ตารางยังใช้ CSV และข้อมูลเดิม เริ่ม update_local.cmd บนคอม หรือ GitHub Actions → Update market data (free) → Run workflow ตามโหมดที่เลือก")
        return
    coverage = manifest.get("coverage", {})
    total = coverage.get("universe", 4900)
    st.caption(f"ชุดข้อมูลเผยแพร่ล่าสุด: {thai_time(manifest.get('published_at'))} · เวลาราคาแต่ละตัวดูในตาราง · ไม่ใช่ราคา real-time")
    st.caption(f"มีราคา {coverage.get('prices', 0):,}/{total:,} · ตรวจราคาสำเร็จในวันตลาดที่สร้างชุด {coverage.get('checked_today', 0):,} · มีอุตสาหกรรม {coverage.get('industry', 0):,} · มีประวัติปันผลที่ตรวจแล้ว {coverage.get('dividends', 0):,}")
    pending = manifest.get("bootstrap_pending", 0)
    if pending:
        st.caption(f"ยังเตรียมราคา/ข้อมูลพื้นฐาน/ปันผลไม่ครบ {pending:,} ตัว งานรอบถัดไปจะทำต่ออัตโนมัติ ข้อมูลที่มีใช้ได้ทันที และ — หมายถึงยังไม่มีข้อมูลเพียงพอ")
    report = manifest.get("report", {})
    if report.get("rate_limited"):
        st.warning("รอบล่าสุดได้ข้อมูลจากผู้ให้บริการไม่ครบ จึงหยุดเรียกและเก็บผลที่สำเร็จไว้ รอบถัดไปจะทำต่อ")
...
@st.cache_resource
def _updater_for_version(version):
    return BackgroundUpdates(get_data_cache(version))


def get_updater(_version=APP_VERSION):
    return _updater_for_version(_version)


get_updater.clear = _updater_for_version.clear
...
def render_scan_controls(csv_frame):
    ...
    remote, _ = get_remote_reader()
    if remote is not None:
        render_prepared_status(remote)
        return universe
...
def watchlist_column_config():
    config = {
        ...
        "Industry": st.column_config.TextColumn("อุตสาหกรรม / หมวด ETF", help="หุ้น: อุตสาหกรรม (Industry) จาก CSV หรือ Yahoo Finance; ETF: หมวดกองทุน (Category) ซึ่งอาจลงทุนหลายอุตสาหกรรม เครื่องหมาย — คือยังไม่มีข้อมูล งานอัตโนมัติจะเติมจากผู้ให้บริการ ข้อมูลนี้อัปเดตแยกจากเวลาราคา", width="medium"),
...
def main():
    ...
    remote, remote_error = get_remote_reader()
    if remote is not None:
        remote.refresh()
    remote_revision = remote.status()["revision"] if remote else 0
    if remote_error:
        st.warning(remote_error)
    st.markdown("""<style>
    ...
    try:
        if upload is not None:
            ...
            prepared_csv, _ = get_data_cache().get("remote:watchlist", request_remote=False)
            if prepared_csv:
                original = parse_watchlist(prepared_csv.encode("utf-8"))
                source_label = "Watchlist เดิมจากชุดข้อมูลที่เตรียมไว้ (GitHub / บนคอม)"
    except Exception as exc:
    ...
    st.caption("รายชื่อครบไม่ได้หมายความว่าราคาครบ ช่องว่างคือยังไม่มีราคาที่โหลดได้ เลือกหุ้นเพื่อวิเคราะห์ได้ทันที งานอัตโนมัติจะเติมข้อมูลต่อเมื่อเชื่อม GitHub หรือเปิดงานอัปเดตบนคอมแล้ว")
    ...
    remote_state = remote.status() if remote else {"busy": False, "revision": 0}
    if remote and not remote_state["busy"] and remote_state["revision"] != remote_revision:
        st.rerun()
    if update_state["busy"] or remote_state["busy"] or auto or remote:
        try:
            from streamlit_autorefresh import st_autorefresh
            st_autorefresh(interval=2000 if update_state["busy"] or remote_state["busy"] else 300_000, key="background_data_poll")
        except ImportError:
            st.info("งานยังโหลดเบื้องหลังได้ กดแสดงผลล่าสุดเพื่อดูข้อมูล หรือเพิ่ม streamlit-autorefresh ใน requirements.txt")
    if st.button("แสดงผลล่าสุดที่โหลดเสร็จ", key="show_saved_results") and remote:
